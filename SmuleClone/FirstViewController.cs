...
namespace SmuleClone
{
	public partial class FirstViewController : UIViewController
	{
		...
		// Add the following class-level field because it will be used in several methods  CLLocationManager locationManager = new CLLocationManager (); 
		...
		public override void ViewDidLoad()
		{
			...
			// Add code for initializing the map here 
			// This is only supported on iOS 8 + 
			if (UIDevice.CurrentDevice.CheckSystemVersion(8, 0))
			{
				locationManager.RequestWhenInUseAuthorization();
			}
			map.MapType = MKMapType.Standard;
			// map.MapType = MKMapType.Satellite; // this is redundant because we have it set in the storyboard 

			// map.MapType = MKMapType.Hybrid; 
			map.ShowsUserLocation = true; // this is redundantly set in the storyboard 
										  // TODO: show the username of the logged in user. This property was set on the LoginViewController code 
		...
		{
			//"Marquette University"//43.038702, -87.929728  
			//"Harley-Davidson Museum"// 43.031892, -87.916508  
			//"Miller Park"//43.028150, -87.971097 
			//"Bradley Center"//43.043914, -87.917262 
		};
...
private void SetLocationAnnotation()
		{
			...
			// based on the selected location, set the Geo-location coordinates  
			...
			{
				// TODO: set the name of the location as the annotation title  Title= selectedLocation, 
...
CGRect((View.Bounds.Width - typesWidth) / 2, View.Bounds.Height - distanceFromBottom, typesWidth, typesHeight));
...
{ 
	...
	return (miles/earthRadius) * radiansToDegrees; 
		...
		public double MilesToLongitudeDegrees(double miles, double atLatitude)
		{
			...
			double radiusAtLatitude = earthRadius * Math.Cos(atLatitude *
		degreesToRadians);
